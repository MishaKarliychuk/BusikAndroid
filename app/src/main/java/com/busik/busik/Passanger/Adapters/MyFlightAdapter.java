...
import com.busik.busik.Passanger.Fragments.ShowArchiveFragment;
...
public class MyFlightAdapter  extends RecyclerView.Adapter<MyFlightAdapter.ViewHolder>{

    private final LayoutInflater inflater;
    private final List<MyFlight> states;
    Date curentDate;
    String token;
    Calendar calCurent;
    boolean isArchived=false;

    public MyFlightAdapter(Context context, List<MyFlight> states,String token,boolean isArchived) {
        this.states = states;
        this.token = token;
    ...
    @Override
    public MyFlightAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @SuppressLint("NewApi")
    @Override
    public void onBindViewHolder(MyFlightAdapter.ViewHolder holder, int position) {
        MyFlight state = states.get(position);
        Date d = null;
...
//        holder.nameDriver.setText(state.getDriver().toString());
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(!isArchived){
                    ((PassangerMainActivity) inflater.getContext()).setIdShowedTrip(state.getId());
                    ((PassangerMainActivity) inflater.getContext()).setIdAppShowed(state.getApplication().getId());
                    ((PassangerMainActivity) inflater.getContext()).replaceFragment(new ShowMyTripReqFragment());
                }
                else {
                    ((PassangerMainActivity) inflater.getContext()).setIdShowedTrip(state.getId());
                    ((PassangerMainActivity) inflater.getContext()).setIdAppShowed(state.getApplication().getId());
                    ((PassangerMainActivity) inflater.getContext()).replaceFragment(new ShowArchiveFragment());
                }
            }
        });
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        TextView nameDriver,departCity,arriveCity,timeDepart,timeArrive,passangerPrice,laggagePrice,tripsDate,rate,status;
